  ...
  function Game() {
    ...
    if (finished) {
      return (
        <div className="text-center text-white p-6">
          ...
          <p className="text-xl">Tu puntuación final es {score} / {questions.length}</p>
          <button
            onClick={() => {
    ...
    return (
      <div className="text-center text-white p-6">
        ...
        {feedback && <p className="mt-4 text-yellow-400 font-semibold">{feedback}</p>}
        <p className="mt-6 text-lg">Puntuación: {score}</p>
      </div>
    );
  }
